...
impl VncInner {
    ...
    async fn input(&mut self, event: X11Event) -> Result<(), VncError> {
        if self.closed {
            ...
            let dimensions = self.screen.load(Ordering::Acquire);
            let screen = ((dimensions >> 16) as u16, dimensions as u16);
            let msg = match event {
    ...
    Fut: Future<Output = Result<(), VncError>>,
{
    ...
    if pf.is_none() {
        if pixel_format.true_color_flag != 1 {
    ...
    loop {
        ...
        match server_msg {
            ServerMsg::FramebufferUpdate(rect_num) => {
                for index in 0..rect_num {
                    let rect = ImageRect::read(stream).await?;
                    if !rect.encoding.qualified()
                        || (rect.encoding != VncEncoding::Raw
                            && !encodings.contains(&rect.encoding))
                    {
                        return Err(VncError::InvalidImageData);
                    }
                    let size = screen.load(Ordering::Acquire);
                    let dimensions = ((size >> 16) as u16, size as u16);
                    if !matches!(
                        rect.encoding,
                        ...
                        crate::limits::rectangle(&rect.rect, dimensions)?;
                    }
                    match rect.encoding {
                        VncEncoding::Raw => {
                            raw_decoder
                        ...
                        VncEncoding::CopyRect => {
                            ...
                            src_rect.x = source_x;
                            src_rect.y = source_y;
                            crate::limits::rectangle(&src_rect, dimensions)?;
                        ...
                        VncEncoding::ExtendedDesktopSizePseudo => {
                            // Layout updates cannot share a framebuffer message with pixels.
                            // Validate the complete bounded message before confirming requests.
                            if index != 0 {
                                return Err(VncError::InvalidImageData);
                            }
